...
class LessoractionController extends Controller {
    ...
    public function actionSearch() {
     ...
     public function actionMyescrow() {
		 if(isset($_POST['actualpaysubmit'])){
    ...
    public function actionAddrfp() {
        return $this->render('addrfp');
    }
...
if(($propertytype == 1) ||($propertytype == 2)||($propertytype == 3)||($propertytype == 4)||($propertytype == 5)||($propertytype == 6)||($propertytype == 7 )||($propertytype == 8)||($propertytype == 9)||($propertytype == 10)){
 //property Details
                        ...
                        $modelid =  $this->insertintoprperty($propertyfeatures,$propertyfor,$propertytype,$city,$state,$country,$location,$locality,$address,$totalarea,$builtuparea,$carpetarea,$expectedprice,$pricepersqft,$maintaincecost,$maintainceper,$includestampcharges,$notwantbroker,$transactiontype,$possessionstatus,$availablity_month,$availablity_year ,$building_name1,$building_no2 ,$totalrooms3,$totalbalconies4 ,$furnishedstatus5 ,$totalfloors6,$floorno7,"","","","","","");
...
$root = $_SERVER['DOCUMENT_ROOT'];
     ...
     * @return mixed
     */
public function insertintoprperty($propertyfeatures,$propertyfor,$propertytype,$city,$state,$country,$location,$locality,$address,$totalarea,$builtuparea,$carpetarea,$expectedprice,$pricepersqft,$maintaincecost,$maintainceper,$includestampcharges,$notwantbroker,$transactiontype,$possessionstatus,$availablity_month,$availablity_year ,$building_name1,$building_no2 ,$totalrooms3,$totalbalconies4 ,$furnishedstatus5 ,$totalfloors6,$floorno7,$officespaceshared8,$personalwashrooms9,$pantryavail10,$cornershop11,$onmainroad12 ,$bundrywallmade13)
{
	$checkifalreadyexitst = \common\models\Property::find()->where(['userid'=>Yii::$app->user->identity->id,'roleid'=>6,'isactive'=>1,'projectypeid'=>$propertytype,'property_features'=>$propertyfeatures,'property_for'=>"Rent",'address'=>$address,'location'=>$location])->one();
	if(!$checkifalreadyexitst){
            $model = new \common\models\Property();
            $model->userid=Yii::$app->user->identity->id; 
            $model->roleid=6;  
            $model->projectypeid  =$propertytype;     
            $model->property_features=$propertyfeatures;		
            $model->propertydescr="Rent"; 
            ...
            $model->save();
			\common\models\activemode::raiseinvoiceforbrokerage($model->userid,$model->id,$model->expected_price);
return $model->id;
	}
}
