...
if(!isset($_SESSION)) 
    ...
    if(isset($_SESSION['locationspost'])){   
        ...
        $getlocality = $_SESSION['locationspost'];
        $town = $_SESSION['townpost'];
        $sector = $_SESSION['sectorpost'];
        $propareaminimum = $_SESSION['areaminpost'];
        $propareamaximum = $_SESSION['areamaxpost'];
        $proppriceminimum = $_SESSION['priceminpost'];
        $proppricemaximum = $_SESSION['pricemaxpost'];
        $proptype = $_SESSION['proptypepost'];
        $propbid = $_SESSION['propbidpost'];
        
        
         $type = $_SESSION['shapedpost'];

         if($type == 'polygon'){

            $geometry = $_SESSION['newspathspost'];
            $radius='';
         }
         if($type == 'circle'){

            $geometry = $_SESSION['centercoordinatespost'];
            $radius = $_SESSION['totalradiuspost'];
         }
         if($type == 'rectangle'){

            $geometry = $_SESSION['newkumapost'];
            $radius ='';
         }
         if($type == 'blank'){
   ...
   <div class="container-fluid no_pad buyer_result">
    ...
    <?php
		...
		<ul class="users_search">
			<li class="user_filt locality_area"><span id="locality_areas"><?php echo $town.' '.$sector; ?></span><span><img src="<?= Yii::getAlias('@frontendUrl').'/newimg/img/down.svg';  ?>" width="17"></li>
			<li class="user_filt prop_type"><span id="prop_types"><?php  echo ($data['typename'] != '' ? $data['typename'] : 'Property Type'); ?></span><span><img src="<?= Yii::getAlias('@frontendUrl').'/newimg/img/down.svg';  ?>" width="17"></li>
			<li class="user_filt prop_area"><span id="prop_areas"><?php echo ($propareaminimum != '' ? $propareaminimum : 'Min Area'); ?> - <?php echo ($propareamaximum != '' ? $propareamaximum : 'Max Area'); ?> Sq. ft.</span><span><img src="<?= Yii::getAlias('@frontendUrl').'/newimg/img/down.svg';  ?>" width="17"></li>
			<li class="user_filt prop_price"><span id="prop_prices"><?php echo  ($proppriceminimum != '' ? $proppriceminimum : 'Min Price'); ?> - <?php echo ($proppriceminimum != '' ? $proppriceminimum : 'Max Area'); ?> </span><span><img src="<?= Yii::getAlias('@frontendUrl').'/newimg/img/down.svg';  ?>" width="17"></li>
		</ul>
...
function applyfilters(){
 ...
 $('#prop_areas').text(areamin+' - '+areamax+' Sq. ft.');
 $('#prop_prices').text(pricemin+' - '+pricemax);
 ///var typeprop =  $("#"+proptype).html();
//$('.prop_type').text(typeprop);
...
<script>
 ...
 $( ".select_who" ).click(function() {
    if ( $('.collapse').hasClass( "in" ) ) {
        $('.collapse').removeClass('in');
        $('.role_type').addClass( "in" );
    }
    if ( $('.more-less').hasClass( "glyphicon-minus" ) ) {
	//alert();
        $('.more-less').removeClass("glyphicon-minus");
       // $('.role_plus').removeClass( "glyphicon-plus" );
        $('.role_plus').addClass( "glyphicon-minus" );
    }
});
 $( ".locality_area" ).click(function() {
    if ( $('.collapse').hasClass( "in" ) ) {
        $('.collapse').removeClass('in');
        $('.location_edit').addClass( "in" );
    }
    if ( $('.more-less').hasClass( "glyphicon-minus" ) ) {
        $('.more-less').removeClass('glyphicon-minus');
        $('.loc_plus').addClass( "glyphicon-minus" );
    }
});
 $( ".prop_type" ).click(function() {
    if ( $('.collapse').hasClass( "in" ) ) {
        $('.collapse').removeClass('in');
        $('.propert_select').addClass( "in" );
    }
    if ( $('.more-less').hasClass( "glyphicon-minus" ) ) {
        $('.more-less').removeClass('glyphicon-minus');
        $('.prop_plus').addClass( "glyphicon-minus" );
    }
});
 $( ".prop_area" ).click(function() {
    if ( $('.collapse').hasClass( "in" ) ) {
        $('.collapse').removeClass('in');
        $('.area_total').addClass( "in" );
    }
    if ( $('.more-less').hasClass( "glyphicon-minus" ) ) {
        $('.more-less').removeClass('glyphicon-minus');
        $('.area_plus').addClass( "glyphicon-minus" );
    }
});
 $( ".prop_price" ).click(function() {
    if ( $('.collapse').hasClass( "in" ) ) {
        $('.collapse').removeClass('in');
        $('.total_price').addClass( "in" );
    }
    if ( $('.more-less').hasClass( "glyphicon-minus" ) ) {
        $('.more-less').removeClass('glyphicon-minus');
        $('.price_plus').addClass( "glyphicon-minus" );
    }
});
