    ...
    public function rules()
    {
        return [
           ...
           // [['user_id', 'role_id', 'project_type_id', 'total_plot_area', 'expected_price', 'price_sq_ft', 'price_acres', 'buildup_area', 'carpet_area', 'total_floors', 'property_on_floor', 'floors_allowed_construction', 'bedrooms', 'bathrooms', 'balconies', 'parking'], 'integer'],
            [['asking_rental_price','maintenance_cost','annual_dues_payable','total_plot_area','buildup_area','carpet_area','total_floors', 'bedrooms','bathrooms','balconies','servant_room','expected_price','membership_charge','expected_rental','property_on_floor'], 'integer','message' => '{attribute} is invalid.'],
		   
            [['request_for', 'locality', 'address', 'plot_unit', 'price_negotiable', 'revenue_lauout', 'present_status', 'shed_RCC', 'maintenance_by', 'availability', 'age_of_property', 'possesion_by', 'rental_type', 'ownership', 'facing', 'LOAN_taken', 'build_unit', 'carpet_unit', 'pooja_room', 'study_room', 'servant_room', 'other_room', 'furnished_status', 'is_active', 'jurisdiction_development'], 'string','message' => '{attribute} is invalid.'],
            ...
            [['project_type_id', 'request_for','city', 'locality', 'address','expected_price', 'price_sq_ft','possesion_by','ownership','LOAN_taken'], 'required','on'=>'creates'],
            
            [['locality','address','ownership'], 'filter', 'filter' => function ($value) {
              return \yii\helpers\HtmlPurifier::process($value);
            }], 

            [['project_type_id', 'request_for', 'city', 'locality', 'address','ownership','available_from','price_sq_ft','LOAN_taken','availability','asking_rental_price'], 'required','on'=>'create'],
             
            
            [['available_date'], 'default', 'value' => null],
      
      
             [['total_plot_area','buildup_area','carpet_area','asking_rental_price','total_floors','maintenance_cost','annual_dues_payable','bedrooms','bathrooms','balconies','servant_room','parking','expected_price','membership_charge','expected_rental','property_on_floor'],'match','pattern'=>"/[0-9]+(\.[0-9][0-9]?)?/"],
...
['buildup_area', 'compare', 'compareAttribute' => 'carpet_area', 'operator' => '>','message' => 'Build up Area should be Greater then Carpet Area.','type' => 'number',],	
    ...
    [['total_plot_area','plot_unit'] ,'required','when'=>function($model){
                                 ...
                                 if($model->project_type_id == 14){
                                   return $model->project_type_id == 14 ;  
                                 }
                                 ...
                                 if($model->project_type_id == 10){
                                   return $model->project_type_id == 10;  
                                     ...
                                     if($('#project_type').val() == '14'){
                                     return $('#project_type').val() == '14';
                                     }
    ...
    [['buildup_area','build_unit'] ,'required','when'=>function($model){
            ...
            [['price_sq_ft'] ,'required','when'=>function($model){
                                 if($model->project_type_id == 11){
                                   return $model->project_type_id == 11 ;  
                                 }
                                 if($model->project_type_id == 12){
