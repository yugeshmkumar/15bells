...
$employecount = \Yii::$app->db->createCommand("SELECT count(*) from company_emp where name='CSR Demand'")->queryAll();
            $findcsr = \Yii::$app->db->createCommand("SELECT * from company_emp where name='CSR Demand' order by alloted asc limit 1")->queryOne();
$findcsrst = \Yii::$app->db->createCommand("SELECT * from company_emp where name='CSR Demand' order by alloted desc limit 1")->queryOne(); 
...
$givezero = Yii::$app->db->createCommand()->update('company_emp', ['alloted' => '0'],'name = "CSR Demand"')->execute();            
            
            $findcsrs = \Yii::$app->db->createCommand("SELECT * from company_emp where name='CSR Demand' order by alloted asc limit 1")->queryOne();  
$findcsrsd = \Yii::$app->db->createCommand("SELECT * from company_emp where name='CSR Demand' order by alloted desc limit 1")->queryOne();            
...
?>
                                                    ...
                                                    <select id="employeeid" name="employeeid" class="form-control" onChange='adddescriptn(this.value)'>
                                                            ...
                                                            <option value="<?php echo $newid ?>"><?php echo 'CSR Demand ('.$newid.')' ?></option>
