                ...
                'rules' => [
                    ...
                    [
                        'actions' => ['logout', 'update', 'index', 'create', 'view', 'delete', 'virtual', 
                            ...
                            'checkstatus', 'showamount1', 'seller', 'maxbidders', 'minraise', 'starttime','ajaxtime','test','test1','getactiveuser','chat','dynamic'],
                        'allow' => true,
                        'roles' => ['@'],
                    ],
                ],
...
$send=Yii::$app->user->identity->id;
 echo $sql="insert into chat_history(send_by,message) values ($send,'$chat') ";
...
<td align=center><b>Bid Date</b></td>
 <td align=center><b>Status</b></td>";
       ...
       public function actionMaxbidders() {
        $connection = Yii::$app->getDb();
        $model = new Transaction();
        $sql = "select max(t.bid_amount) as bidder,u.aliasname  from transaction t inner join user u on u.id=t.buyer_id where t.status ='Approved' group by buyer_id";
        $connection = Yii::$app->getDb();
