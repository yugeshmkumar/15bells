...
return $r_res=$result_chk['bid_amount'];
	...
	{
		$connection = Yii::$app->getDb();
		 $count="SELECT count( DISTINCT(buyer_id) ) as total FROM transaction  ";
 ...
 return  $getcount=$count['total'];
	...
	{
	$sql="select todatetime from vr_setup";
	$connection = Yii::$app->getDb();
  ...
  return   $type= $result['type'];
	...
	{
     $chk_user=Yii::$app->user->identity->id; 
   $sql="select status from transaction where buyer_id=$chk_user order by id desc limit 0,1";
   $connection = Yii::$app->getDb();
   $command = $connection->createCommand($sql);
   $result = $command->queryOne();
...
public function getCurrentactivetime()
	{
	 return "Y-m-d H:i:s";
        ...
        public function getAddminutes($min)
	{
	$sql="select todatetime from vr_setup";
	$connection = Yii::$app->getDb();
	$command = $connection->createCommand($sql);
	...
	$result = $command->queryOne();
     ...
     //    $command_get->query();
        ...
        public function getWinner() {
            $sql="select username from user where id=(Select buyer_id from transaction where bid_amount = (select max(bid_amount) from transaction where status='Approved')) ";
            $connection = Yii::$app->getDb();
	$command = $connection->createCommand($sql);
	$result = $command->queryOne();
	 return $result['username'];
        ...
        public function getWinnerid() {
            $sql="select id from user where id=(Select buyer_id from transaction where bid_amount = (select max(bid_amount) from transaction where status='Approved')) ";
            $connection = Yii::$app->getDb();
	$command = $connection->createCommand($sql);
	$result = $command->queryOne();
	 return $result['id'];
