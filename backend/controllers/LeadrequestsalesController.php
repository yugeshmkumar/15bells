 ...
 */
class LeadrequestsalesController extends Controller
{
...
if($assigndash->item_name == "sales_supply_lessor"){
    ...
    if (isset($_POST['hasEditable'])) {
        ...
        if (isset($_POST['editableKey'])) {
			...
			$tagsdata = $_POST['Leads'][$editableIndex]['tags'];
     ...
     public function actionSupplyindex()
    {
        $model = new \common\models\Leads(); // your model can be loaded here
    ...
    if (isset($_POST['hasEditable'])) {
     ...
     return $this->render('supplyindex', [
...
$message=$_GET['message'];
		...
		$arrfindallstatus = \common\models\LeadcurrentstatusSales::find()->where(['leadid'=>$leadid])->all();
			foreach($arrfindallstatus as $findallstatus){
			$findallstatus->isactive=0;
			$findallstatus->save();
			
			$newModel3 = new \common\models\LeadcurrentstatusSales();
...
mail($to, $subject, $message, $headers);
	...
	public function actionCreateanorderaction()
	{
		...
		$LeadcurrentstatusSales = \common\models\LeadcurrentstatusSales::find()->where(['leadid'=>$leadid,'isactive'=>1])->one();
		$Leadsproduct = \common\models\Leadsproduct::find()->where(['id'=>$LeadcurrentstatusSales->productid])->one();
		
		//add in order for finance
		$OrderCustomer = new \common\models\OrderCustomer();
		$OrderCustomer->leadID =$leadid;
		$OrderCustomer->productid=$LeadcurrentstatusSales->productid;
		$OrderCustomer->orderdesc=$message;
		$OrderCustomer->fromdate=$fromdate;
		$OrderCustomer->todate=$todate;
		$OrderCustomer->message=$message;
		$OrderCustomer->tempinvoiceno=$LeadcurrentstatusSales->productid.'00'.$leadid.'INV-16';
		...
		$Project->productid=$LeadcurrentstatusSales->productid;
		...
		$newModel3 = new \common\models\LeadcurrentstatusSales();
				$newModel3->leadid =$leadid;
				$newModel3->productid=$LeadcurrentstatusSales->productid;
...
public function actionReverselead()
{
	$leadid = $_GET['leadid'];
		 ...
		 $arrLeadcurrentstatusSales = \common\models\LeadcurrentstatusSales::find()->where(['leadid'=>$leadid])->all();
		 foreach($arrLeadcurrentstatusSales as $LeadcurrentstatusSales){
			 $LeadcurrentstatusSales->isactive=0;
			  $LeadcurrentstatusSales->save();
		 }
		  $leadoldcurrentstatus = \common\models\LeadcurrentstatusSales::find()->where(['leadid'=>$leadid])->one();
		 $Leadcurrentnewstatus = new \common\models\LeadcurrentstatusSales();
...
$this->SendHTMLMail($to,$subject,$content);	
	 ...
	 public function actionAssignleadaction(){
		 ...
		 $Assignleadtoemployee = \common\models\LeadassignmentSales::find()->where(['leadid'=>$leadid,'isactive'=>1])->one();
...
$this->SendHTMLMail($to,$subject,$content);	
    ...
    public function actionUpdate($id)
    {
		$id = $_GET['id'];
        $model = $this->findModel($id);
		$getcompanyid =\common\models\LeadcurrentstatusSales::find()->where(['leadid'=>$id,'isactive'=>1])->one();
      ...
      $model2 = $this->findModel1($getcompanyid->productid);
		  $model3 = $this->findModel2($leadRequestID->company);
           ...
           $modelreq3=$request['Company'];
            ...
            $leadRequestID->save();
			$arrfindallstatus = \common\models\LeadcurrentstatusSales::find()->where(['leadid'=>$leadRequestID->leadRequestID])->all();
			foreach($arrfindallstatus as $findallstatus){
			$findallstatus->isactive=0;
			$findallstatus->save();
			}
